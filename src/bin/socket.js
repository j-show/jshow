...
const createPlugins  = function (plugins) {
	  ...
	  createModule   = function (root, name, plugins) {
	  ...
	  createTemplate = function (root, name) {
		  func.mkdir(root, this.root);
	  ...
	  createPackage  = function (root, cfg, name) {
		  const pkg = {
			  name:         name,
			  version:      "0.0.1",
			  private:      true,
...
exports = module.exports = function (program, f) {
	...
	program
		...
		.action(function (dir, opt) {
			...
			let force = opt.force,
				name  = typeof(opt.name) === "string" ? opt.name : path.basename(dir),
				cfg   = {
					name:     name,
					module:   opt.module,
					type:     opt.type || "koa",
			...
			package(opt.package, cfg);

			func.empty(dir, function (empty) {
