 ...
 * ==========================================
 */
define("Security", ["Conver"], function (require, module, CONVER) {
	"use strict";
	...
	const api = {
			...
			if (enc === true) {
				...
				data = (value => {
					...
					for (let i = 0, l = value.length, c1, c2, c3; i < l;) {
						...
						i += 1;
						if (i === l) {
							result.push(char.charAt(c1 >> 2));
							result.push(char.charAt((c1 & 0x3) << 4));
							result.push("==");
							break;
						}
						...
						if (i === l) {
							result.push(char.charAt(c1 >> 2));
							result.push(char.charAt(((c1 & 0x3) << 4) | ((c2 & 0xF0) >> 4)));
							result.push(char.charAt((c2 & 0xF) << 2));
							result.push("=");
							break;
						}
						...
						result.push(char.charAt(c1 >> 2));
						result.push(char.charAt(((c1 & 0x3) << 4) | ((c2 & 0xF0) >> 4)));
						result.push(char.charAt(((c2 & 0xF) << 2) | ((c3 & 0xC0) >> 6)));
						result.push(char.charAt(c3 & 0x3F));
					}
				...
				data = (value => {
					...
					for (let i = 0, l = value.length, c1, c2, c3, c4; i < l;) {
						do {
							c1 = char[value.charCodeAt(i) & 0xFF];
							i += 1;
						}
						while (i < l && c1 === -1);
						if (c1 === -1) break;
						...
						do {
							c2 = char[value.charCodeAt(i) & 0xFF];
							i += 1;
						}
						while (i < l && c2 === -1);
						if (c2 === -1) break;

						result.push(String.fromCharCode((c1 << 2) | ((c2 & 0x30) >> 4)));

						do {
							c3 = value.charCodeAt(i) & 0xFF;
							i += 1;

							if (c3 === 61) return result.join("");

							c3 = char[c3];
						}
						while (i < l && c3 === -1);
						if (c3 === -1) break;
						result.push(String.fromCharCode(((c2 & 0xF) << 4) | ((c3 & 0x3C) >> 2)));

						do {
							c4 = value.charCodeAt(i) & 0xFF;
							i += 1;

							if (c4 === 61) return result.join("");
							c4 = char[c4];
						}
						while (i < l && c4 === -1);
						if (c4 === -1) break;
						result.push(String.fromCharCode(((c3 & 0x03) << 6) | c4));
					}

					return result.join("");
				})(data);
				data = CONVER.toHex(data, "utf16");
				data = CONVER.toString(data, arg);
			}
		...
		MD5 (value, simple = false, callback = simple) {
			...
			const toWordArray  = value => {
				let ml    = value.length;
				let lw_t1 = ml + 8;
				let lw_t2 = (lw_t1 - (lw_t1 % 64)) / 64;
				let nw    = (lw_t2 + 1) * 16;
				let bp    = 0;
				let bc    = 0;
				let r     = new Array(nw - 1);
				let wc;
			...
			const toUTF8Encode = value => {
				...
				for (let i = 0, c; i < data.length; i++) {
					...
					if (c < 0x80) {
						...
						r.push(String.fromCharCode((c >> 6) | 0xC0));
						r.push(String.fromCharCode((c & 0x3F) | 0x80));
					}
					else {
						r.push(String.fromCharCode((c >> 12) | 0xE0));
						r.push(String.fromCharCode(((c >> 6) & 0x3F) | 0x80));
						r.push(String.fromCharCode((c & 0x3F) | 0x80));
					}
				}
			...
			const toF          = (x, y, z) => (x & y) | (~x & z);
			const toG          = (x, y, z) => (x & z) | (y & ~z);
			const toH          = (x, y, z) => x ^ y ^ z;
			const toI          = (x, y, z) => y ^ (x | ~z);
			const toFF         = (a, b, c, d, x, s, ac) => toUnsigned(RotateLeft(toUnsigned(a, toUnsigned(toUnsigned(toF(b, c, d), x), ac)), s), b);
			const toGG         = (a, b, c, d, x, s, ac) => toUnsigned(RotateLeft(toUnsigned(a, toUnsigned(toUnsigned(toG(b, c, d), x), ac)), s), b);
			const toHH         = (a, b, c, d, x, s, ac) => toUnsigned(RotateLeft(toUnsigned(a, toUnsigned(toUnsigned(toH(b, c, d), x), ac)), s), b);
			const toII         = (a, b, c, d, x, s, ac) => toUnsigned(RotateLeft(toUnsigned(a, toUnsigned(toUnsigned(toI(b, c, d), x), ac)), s), b);
			...
			const D = toWordArray(toUTF8Encode(data));
			...
			return $.isFunction(callback) ? callback(result, data) : result;
		},
		/**
		 * SHA encoder
		 *
		 ...
		 *    @param {string} [opt.format=TEXT] <input format mode>
		 *    @param {string} [opt.encoding=UTF8] <input and output encoding>
		 *    @param {string} [opt.inf=TEXT] <input format mode>
		 *    @param {string} [opt.ine=UTF8] <input encoding>
		 *    @param {string} [opt.outf=HEX] <output format mode>
		 *    @param {string} [opt.oute=UTF8] <output encoding>
		 *    @param {number} [opt.level=256] <encoder level>
		 *    @param {string} [opt.key] <encoder key>
		 * @param {function} [callback]
		 * @returns {string}
		 */
		SHA (value, opt, callback = opt) {
			const int_64       = function (msint_32, lsint_32) {
				this.highOrder = msint_32;
				this.lowOrder  = lsint_32;
			};
			const str2binb     = (src, buf, len, encoding) => {
			...
			const rotl_32      = (x, n) => (x << n) | (x >>> (32 - n));
			const rotr_32      = (x, n) => (x >>> n) | (x << (32 - n));
			const rotr_64      = (x, n) => {
			...
			const parity_32    = (x, y, z) => x ^ y ^ z;
			const ch_32        = (x, y, z) => (x & y) ^ (~x & z);
			const ch_64        = (x, y, z) => {
				return new int_64(
			...
			const maj_32       = (x, y, z) => (x & y) ^ (x & z) ^ (y & z);
			const maj_64       = (x, y, z) => {
				return new int_64(
					(x.highOrder & y.highOrder) ^
					(x.highOrder & z.highOrder) ^
			...
			const sigma0_32    = (x) => rotr_32(x, 2) ^ rotr_32(x, 13) ^ rotr_32(x, 22);
			const sigma0_64    = (x) => {
			...
			const sigma1_32    = (x) => rotr_32(x, 6) ^ rotr_32(x, 11) ^ rotr_32(x, 25);
			const sigma1_64    = (x) => {
			...
			const gamma0_32    = (x) => rotr_32(x, 7) ^ rotr_32(x, 18) ^ shr_32(x, 3);
			const gamma0_64    = (x) => {
			...
			const gamma1_32    = (x) => rotr_32(x, 17) ^ rotr_32(x, 19) ^ shr_32(x, 10);
			const gamma1_64    = (x) => {
			...
			const finalizeSHA1 = (buf, bLen, pLen, H) => {
			...
			const finalizeSHA2 = (buf, bLen, pLen, H, level) => {
				let offset,
					binaryStringInc;
			...
			const setKey       = (key, level) => {
				...
				if (blockByteSize < i) {
					buf = finalizeFunc(buf, len, 0, getH(data), level);
			...
			if (!$.has(level, [1, 224, 256, 384, 512])) level = 256;
			if (!$.isString(key)) key = "";
			if ($.has(format, ["TEXT", "B64", "BASE64", "HEX"])) format = "TEXT";
			if ($.has(encoding, ["ANSI", "UTF8", "UTF16"])) encoding = "UTF8";

			if (!$.has(inf, ["TEXT", "B64", "BASE64", "HEX"])) inf = format;
			if (!$.has(inf, ["TEXT", "B64", "BASE64", "HEX"])) inf = "TEXT";
			if (!$.has(outf, ["HEX", "B64", "BASE64"])) outf = "HEX";
			if (!$.has(ine, ["ANSI", "UTF8", "UTF16"])) ine = encoding;
			if (!$.has(oute, ["ANSI", "UTF8", "UTF16"])) oute = encoding;

			if (!$.has(ine, ["ANSI", "UTF8", "UTF16"])) ine = "UTF8";
			if (!$.has(oute, ["ANSI", "UTF8", "UTF16"])) oute = "UTF8";
			...
			let finalizeFunc = finalizeSHA2;
			let informatFunc,
				outformatFunc;

			let variantBlockSize = 512;
			let outputBinLen     = level;
			let intermediateH    = getH(level);
		...
		CRC (value, type, mode) {
			...
			switch ($.type(data, true)) {
				...
				case "array":
					break;
				case "string":
					if (data) data = CONVER.toHex(data, "ansi");
					else data = [];
					break;
			}
